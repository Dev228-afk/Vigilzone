...
class FrameDetectionCache:
    ...
    def get_latest(self, key: str) -> Optional[Any]:
        """Return the most recent cached value for *key* regardless of frame_seq.

        Used by downstream lanes (violence_candidate, weapon_yolo) that need
        person detections but don't track the current frame sequence number.
        """
        with self._lock:
            entry = self._store.get(key)
            if entry is not None:
                return entry[1]  # (frame_seq, detections) → detections
            return None
