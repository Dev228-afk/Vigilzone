...
class YOLOv8FallbackLane(BaseLane):
    ...
    def infer(self, frame_bgr: np.ndarray, ts_utc: str,
        ...
        if self._det_cache is not None and frame_seq > 0:
            ...
            # Also store per-camera in structured format for violence/weapon lanes
            self._det_cache.put(
                self.camera_id, frame_seq,
                {"detections": [
                    {"label": "person", "bbox": box, "conf": conf}
                    for box, conf in person_boxes_for_cache
                ]},
            )
