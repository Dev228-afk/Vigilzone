...
│   ├── models.yaml         # Model paths, thresholds, runtime budgets
│   ├── policy.yaml         # Identity-driven severity/suppress rules
...
│   ├── incidents/          # incident framework (base, state, registry)
│   ├── ingest/             # OpenCV, FFmpeg, live_camera readers
│   ├── lanes/              # 14 detection lanes
│   ├── logic/              # aggregator, voting, cooldown, deduper, zones
│   ├── runtime/            # device selection, GPU scheduler, auto-throttle
│   └── services/           # AnyAnomaly client/worker
...
├── backend/                # Django REST backend (separate service)
├── frontend/               # React dashboard (separate service)
├── third_party/            # External repos (AnyAnomaly)
├── tools/                  # Utility scripts (calibration, ONNX→TRT)
├── run.py                  # Entry point
...
- **FFmpeg** – [download](https://ffmpeg.org/download.html), add `bin/` to PATH, verify with `ffmpeg -version`
- **PyTorch** – see step 1 below
...
Edit `configs/cameras.yaml` to add RTSP cameras or use a local video file:

```yaml
cameras:
  - camera_id: test_cam
    rtsp_url: "test_video.mp4"      # or "C:/path/to/video.mp4"
...
```bash
python run.py            # or double-click run_ai_module.bat on Windows
```
...
# Verify dependencies (quick self-test)
python validate.py
```
...
| Problem | Solution |
|---------|----------|
| "Module not found" | Make sure you're in `ai_module/` and ran `pip install -r requirements.txt` |
| "Model weights not found" | Drop `.pt`/`.onnx` files in `models/`; update `models.yaml` |
